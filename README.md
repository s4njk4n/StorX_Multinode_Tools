...
* You will need a computer (running Ubuntu 20.04) that these scripts will be installed on (eg. laptop, desktop, virtual machine, VPS). If you are using
  the Plugin Multinode Tools as well, then these scripts can all be installed on the same computer. 
...
3. Clone the scripts repository (Note: Preserves any preexisting ip_addresses file in local git repo. All other files will in your local repo directory
will be deleted and the current repo will then be downloaded):
